    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            SELECT 
                ...
                xrp_avg_buy_price, performance, fee, settlement_amount
...
def create_ripple_price_chart():
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="<b>%{x}</b><br>가격: %{y:,.2f} KRW<extra></extra>"
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="6시간 이동평균",
            hovertemplate="<b>%{x}</b><br>MA6: %{y:,.2f} KRW<extra></extra>"
        )
    )
    ...
    if xrp_balance > 0 and xrp_avg_buy_price > 0:
        ...
        fig.add_annotation(
            x=df_price.index[-1],
            y=xrp_avg_buy_price,
            text=f"평균 매수가: {xrp_avg_buy_price:,.2f}원",
            showarrow=True,
            arrowhead=2,
            arrowcolor="#FF9800",
            arrowsize=1,
    ...
    if target_data is not None:
        ...
        if pd.notna(target_data['entry_price1']):
            ...
            right_annotations.append({
                ...
                "text": f"1차 진입가: {target_data['entry_price1']:,.2f}원 ({target_data['entry_percentage1']}%)",
                "color": COLOR["accent"]
            })
        ...
        if pd.notna(target_data['entry_price2']):
            ...
            right_annotations.append({
                ...
                "text": f"2차 진입가: {target_data['entry_price2']:,.2f}원 ({target_data['entry_percentage2']}%)",
                "color": COLOR["light"]
            })
        ...
        if pd.notna(target_data['target1_price']):
            ...
            right_annotations.append({
                ...
                "text": f"1차 목표가: {target_data['target1_price']:,.2f}원 ({target_data['target1_sell_pct']}%)",
                "color": COLOR["success"]
            })
        ...
        if pd.notna(target_data['target2_price']):
            ...
            right_annotations.append({
                ...
                "text": f"2차 목표가: {target_data['target2_price']:,.2f}원 ({target_data['target2_sell_pct']}%)",
        ...
        right_annotations.append({
            ...
            "text": f"손절가: {target_data['stop_loss_price']:,.2f}원",
            "color": COLOR["danger"]
        })
...
def display_target_info():
    ...
    if pd.notna(target_data['entry_price1']):
        ...
        entry_data.append({
            "구분": "1차 진입가",
            "가격": format_large_number(target_data['entry_price1']),
    ...
    if pd.notna(target_data['entry_price2']):
        ...
        entry_data.append({
            "구분": "2차 진입가",
            "가격": format_large_number(target_data['entry_price2']),
    ...
    if pd.notna(target_data['target1_price']):
        ...
        target_data_list.append({
            "구분": "1차 목표가",
            "가격": format_large_number(target_data['target1_price']),
    ...
    if pd.notna(target_data['target2_price']):
        ...
        target_data_list.append({
            "구분": "2차 목표가",
            "가격": format_large_number(target_data['target2_price']),
    ...
    if pd.notna(target_data['target3_price']):
        ...
        target_data_list.append({
            "구분": "3차 목표가",
            "가격": format_large_number(target_data['target3_price']),
    ...
    target_data_list.append({
        "구분": "손절가",
        "가격": format_large_number(target_data['stop_loss_price']),
        "매도 비율": "100%",
...
def display_asset_overview():
    ...
    krw_percentage = 100 - xrp_percentage
    ...
    with col1:
        ...
        st.markdown(
            create_metric_card(
                ...
                f"{xrp_percentage:.1f}% | {format_large_number(xrp_value)}"
            ), 
            unsafe_allow_html=True
        )
    ...
    # 수익률 계산
    profit_percentage = calculate_upbit_style_profit_percentage(xrp_balance, current_price, xrp_avg_buy_price)
    profit_amount = calculate_profit_amount(xrp_balance, current_price, xrp_avg_buy_price)
    ...
    with col1:
        st.markdown(
            create_metric_card(
                ...
                f"XRP: {format_large_number(round(xrp_value))}"
            ),
            unsafe_allow_html=True
        )
...
def display_asset_details():
    ...
    with col1:
        ...
        st.markdown(
            create_metric_card(
                "보유 XRP",
                f"{xrp_balance:.8f} XRP",
                f"가치: {format_large_number(xrp_value_krw)}"
            ),
            unsafe_allow_html=True
        )
    ...
    with col3:
        st.markdown(
            create_metric_card(
                "XRP 평균 매수가",
                f"{format_large_number(xrp_avg_buy_price)}"
            ),
            unsafe_allow_html=True
        )
...
def display_operation_status(df):
    ...
    with col4:
        ...
        st.markdown(
            create_metric_card(
                "XRP 현재가",
                f"{format_large_number(current_price)}",
                f"{price_change_percentage:.2f}%",
                delta_color
            ),
            unsafe_allow_html=True
    ...
    for idx, row in df_display.iterrows():
        ...
        with tab1:
            if decision_type == 'buy':
                ...
                with col1:
                    st.markdown(
                        create_metric_card("매수가", format_large_number(row['xrp_krw_price'])),
                        unsafe_allow_html=True
                    )
                    st.markdown(
                ...
                with col2:
                    st.markdown(
                        create_metric_card("수량", f"{xrp_amount:.2f} XRP"),
                        unsafe_allow_html=True
                    )
                    st.markdown(
                ...
                with col1:
                    st.markdown(
                        create_metric_card("매도가", format_large_number(row['xrp_krw_price'])),
                        unsafe_allow_html=True
                    )
                    st.markdown(
                        create_metric_card("정산금액", format_large_number(settlement_amount)),
                ...
                with col1:
                    st.markdown(
                        create_metric_card("보유 XRP", f"{row.get('xrp_balance', 0):.2f} XRP"),
                        unsafe_allow_html=True
                    )
                    st.markdown(
                        create_metric_card("현재가", format_large_number(row.get('xrp_krw_price', 0))),
                        unsafe_allow_html=True
                    )
                ...
                with col2:
                    ...
                    st.markdown(
                        create_metric_card("평균매수가", format_large_number(row.get('xrp_avg_buy_price', 0))),
                        unsafe_allow_html=True
                    )
            ...
            with st.expander("📊 상세 거래 정보", expanded=False):
                ...
                with col2:
                    ...
                    st.markdown(
                        create_metric_card(
                            "평균 매수가", 
                            format_large_number(row['xrp_avg_buy_price'])
                        ),
                        unsafe_allow_html=True
                    )
                    st.markdown(
                        create_metric_card(
                            "총 자산 가치", 
                            format_large_number(row['xrp_balance'] * row['xrp_krw_price'] + row['krw_balance'])
                        ),
                        unsafe_allow_html=True
                    )
...
def main():
    ...
    <div class="footer">
        ...
        <p>이 시스템은 XRP(리플) 시장을 자동으로 분석하고 거래 결정을 내립니다.</p>
        <p>© 2025 ARGOS Trading System</p>
    </div>
    """, unsafe_allow_html=True)
