...
# ============================================================================
INITIAL_INVESTMENT = 316543.14  # 초기 투자금액 (원) - 필요시 수정하세요
...
def get_current_portfolio_value():
    try:
        ...
        return total_value, btc_value, krw_balance, current_price
...
def get_real_time_balance():
    try:
        ...
        return btc_balance, krw_balance, btc_avg_buy_price
    except Exception as e:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            required_columns = {
                ...
                'btc_balance', 'krw_balance', 'fee', 'settlement_amount',
                'btc_avg_buy_price', 'btc_krw_price', 'performance'
            }
...
def calculate_simple_profit_metrics():
    ...
    try:
        # 현재 총 자산 가치 조회
        total_value, btc_value, krw_balance, current_price = get_current_portfolio_value()
        ...
        # BTC 관련 정보
        btc_balance, _, btc_avg_buy_price = get_real_time_balance()
        
        # 미실현 손익 (업비트 기준)
        unrealized_pnl = 0
        if btc_balance > 0 and btc_avg_buy_price > 0:
            unrealized_pnl = (float(current_price) - float(btc_avg_buy_price)) * float(btc_balance)
...
def calculate_daily_data(df):
    ...
    for _, row in last_records.iterrows():
        date = row['date']
        btc_balance = float(row.get('btc_balance', 0))
        krw_balance = float(row.get('krw_balance', 0))
        btc_price = float(row.get('btc_krw_price', 0))
...
def display_asset_overview():
    ...
    btc_value = metrics['btc_value']
    krw_balance = metrics['krw_balance']
    
    btc_percentage = (btc_value / total_value * 100) if total_value > 0 else 0
    ...
    with col2:
        delta_text = f"{btc_percentage:.1f}% | {metrics['btc_balance']:.4f} BTC"
        st.markdown(create_metric_card(
            "BTC 자산", 
            format_large_number(btc_value),
            delta_text,
            "primary",
            "🪙"
        ), unsafe_allow_html=True)
    ...
    # BTC 상세 정보
    if metrics['btc_balance'] > 0:
        st.markdown("### BTC 보유 상세")
        
        current_price = metrics['current_price']
        avg_buy_price = metrics['btc_avg_buy_price']
...
def display_profit_details():
    ...
    with info_col3:
        # 자산 구성 비율
        asset_composition = f"BTC {(metrics['btc_value']/current_value*100):.1f}%" if current_value > 0 else "N/A"
        st.markdown(create_metric_card(
            "자산 구성", 
            asset_composition,
...
def create_price_chart():
    create_section_title("BTC 가격 차트", "📈")
    
    try:
        # 24시간 1시간 봉 데이터 조회
        df_price = pyupbit.get_ohlcv("KRW-BTC", interval="minute60", count=24)
        ...
        # 평균 매수가 라인 추가
        if metrics['btc_avg_buy_price'] > 0:
            fig.add_hline(
                y=metrics['btc_avg_buy_price'],
                line_dash="dot",
                line_color="#f59e0b",
                line_width=2,
...
def create_asset_allocation_chart():
    ...
    if total_value > 0:
        ...
        fig = go.Figure(data=[go.Pie(
            labels=['BTC', '현금'],
            values=[btc_percentage, krw_percentage],
            hole=0.6,
            marker=dict(
            ...
            customdata=[format_large_number(btc_value), format_large_number(krw_balance)]
        )])
        ...
        with st.container():
            ...
            with tab1:
                ...
                if decision_type == 'buy':
                    ...
                    btc_amount = settlement_amount / row['btc_krw_price'] if row['btc_krw_price'] > 0 else 0
                    
                    with detail_col1:
                        st.metric("매수가", f"{row['btc_krw_price']:,.0f}원")
                    with detail_col2:
                        st.metric("매수 금액", format_large_number(settlement_amount))
                    with detail_col3:
                        st.metric("매수 수량", f"{btc_amount:.4f} BTC")
                
                elif decision_type == 'sell':
                    settlement_amount = row.get('settlement_amount', 0)
                    percentage = row.get('percentage', 100)
                    btc_amount = (row['btc_balance'] * percentage / 100) if pd.notna(percentage) else 0
                    
                    with detail_col1:
                        st.metric("매도가", f"{row['btc_krw_price']:,.0f}원")
                    with detail_col2:
                        st.metric("매도 금액", format_large_number(settlement_amount))
                    with detail_col3:
                        st.metric("매도 수량", f"{btc_amount:.4f} BTC")
                
                else:  # predict
                    with detail_col1:
                        st.metric("BTC 잔고", f"{row.get('btc_balance', 0):.4f} BTC")
                    with detail_col2:
                        st.metric("현금 잔고", format_large_number(row.get('krw_balance', 0)))
                    with detail_col3:
                        st.metric("BTC 시세", f"{row.get('btc_krw_price', 0):,.0f}원")
